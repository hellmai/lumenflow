...
- id: INV-NO-CONSOLE-LOG
  ...
  pattern: 'console\.log\('
  scope:
    - 'packages/@patientpath/application/src/**/*.ts'
...
- id: INV-NO-INFRA-IN-APP
  ...
  import: '@patientpath/infrastructure'
  scope:
    - 'packages/@patientpath/application/**/*.ts'
...
1. **Architecture boundaries (hexagonal):**
   - Forbid `packages/@patientpath/application/**` importing `@patientpath/infrastructure`

2. **Direct vendor clients in wrong layer:**
   - Forbid `packages/@patientpath/application/**` importing `@supabase/supabase-js`
