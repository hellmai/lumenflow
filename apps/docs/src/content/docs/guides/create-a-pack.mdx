...
<Aside type="tip">
  Use the narrowest scope that works. A read-only tool should never declare `access: write`. For
  network access, prefer `posture: allowlist` over `posture: full` whenever possible -- list only
  the hosts the tool actually needs. The kernel enforces the intersection, so overly broad scopes
