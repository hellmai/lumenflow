...
Use this for commands that check whether the target database is up to date. Do not use it to run
...
<Aside type="tip" title="Added in v3.19.0">
  Conditional commands let you define gates that run only when specific file patterns change,
  avoiding expensive checks on every WU.
</Aside>
