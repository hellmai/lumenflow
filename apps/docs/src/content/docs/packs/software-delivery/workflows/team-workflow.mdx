...
<Aside type="caution" title="wu:brief is mandatory after wu:claim">
  `wu:claim` auto-records a stub event (`claim-auto`), but this **does not** satisfy the `wu:done`
  evidence gate. Agents must run `wu:brief` explicitly after claiming — it generates the handoff
  prompt, checks lane occupation, validates sizing, and records proper evidence. Without an explicit
  `wu:brief`, `wu:done` will block with an actionable error message.
</Aside>
