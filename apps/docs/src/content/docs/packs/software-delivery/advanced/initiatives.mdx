...
This creates `docs/operations/tasks/initiatives/INIT-001.yaml`:
...
### Set Phase Execution Order

```bash
pnpm initiative:edit --id INIT-001 --phase-execution-order sequential
pnpm initiative:edit --id INIT-001 --phase-execution-order parallel
```

### Set Dependency Model

```bash
pnpm initiative:edit --id INIT-001 --dependency-model strict-phase-gate
```
...
This creates a plan template at `docs/operations/plans/INIT-001-<slug>.md` and links it to the initiative via the `related_plan` field.

### Link an Existing Plan

```bash
pnpm initiative:plan --initiative INIT-001 --plan docs/operations/plans/my-plan.md
```
