...
- **Kernel + Pack architecture**: The runtime is now a domain-agnostic kernel that loads pluggable packs. Software Delivery remains the delivery workflow surface, while additional first-party packs extend workspace productivity and governed agent execution.
...
2. **Convert to WU YAML**
   ...
   mkdirSync('docs/operations/tasks/wu', { recursive: true });
   ...
   jiraExport.issues.forEach((issue, index) => {
     ...
     writeFileSync(`docs/operations/tasks/wu/${wuId}.yaml`, toYAML(wu));
   });
...
8. **Freeze old system**
   ...
   > This project now uses LumenFlow. New work: see [repo]/docs/operations/tasks/
...
```bash
# Find WU by Jira ID
grep -r "JIRA-PROJ-123" docs/operations/tasks/wu/
```
...
- `cat docs/operations/tasks/status.md` - Recent completions
...
grep -rh "external_refs" docs/operations/tasks/wu/ | sort | uniq -d
