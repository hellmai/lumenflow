...
The LumenFlow MCP server exposes first-party pack tools and kernel runtime tools to AI assistants via the Model Context Protocol. The majority of the **114 tools** come from pack surfaces, while **7** are kernel runtime task primitives for advanced lifecycle operations.
...
The MCP server exposes **114 tools** organized into 13 categories (107 in the core `allTools` registry plus 7 runtime task tools):
...
The CLI public manifest currently exposes **125 commands across primary, alias, and legacy surfaces**. The MCP registry keeps parity with the primary command surface where that maps cleanly into MCP, adds a small set of MCP-only convenience tools, and includes **7** kernel-native runtime task tools. The `allTools` registry contains **107** tools; `runtimeTaskTools` adds **7** more for **114** registered tools in total.
