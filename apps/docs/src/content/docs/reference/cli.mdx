...
```bash
pnpm pack:install --id <packId> --source <source> --version <version>
```
...
| ---------------------------- | ----------------------------------------------------------------------------- | -------- |
| `-f, --force`                | Force operation                                                               | No       |
| `--id &lt;packId&gt;`        | Pack ID to install                                                            | Yes      |
...
When `--source registry` is used, `--integrity` is optional. If omitted, the
CLI resolves integrity from registry metadata for the requested version before
installing.
...
Non-interactive bootstrap (recommended for first setup):
