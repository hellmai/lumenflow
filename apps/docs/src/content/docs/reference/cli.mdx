...
<Aside type="note" title="Ownership guards (v3.19.0)">
  ...
  allowing state mutations. If the active session differs from the WU's claimed session, you'll see
  a `SESSION OWNERSHIP VIOLATION` or `CLAIM OWNERSHIP VIOLATION` error. Use `--override-owner
  --reason "..."` for manual cross-session recovery. All overrides are audited to
