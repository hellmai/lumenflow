...
Edit plan content. Resolves plan file from initiative `related_plan` metadata, glob matching, or explicit `--file` flag.

```bash
pnpm plan:edit --id <wuId> --section <name> --content <text>
...
| Flag                     | Description                                           | Required |
| ------------------------ | ----------------------------------------------------- | -------- |
| `-i, --id &lt;wuId&gt;`  | Work Unit or Initiative ID (e.g., WU-123, INIT-040).  | Yes      |
| `--section &lt;name&gt;` | Section name to edit (Goal, Scope, Approach, etc.)    | Yes      |
| `--content &lt;text&gt;` | New content for the section                           | No       |
| `--append &lt;text&gt;`  | Content to append to the section (instead of replace) | No       |
| `--file &lt;path&gt;`    | Plan filename or path (resolves relative to plansDir) | No       |
...
Promote plan to approved status. Resolves plan file from initiative `related_plan` metadata, glob matching, or explicit `--file` flag.

```bash
pnpm plan:promote --id <wuId>
pnpm plan:promote --id INIT-040
pnpm plan:promote --id WU-100 --file my-plan.md
```
...
| ----------------------- | ----------------------------------------------------- | -------- |
| `-i, --id &lt;wuId&gt;` | Work Unit or Initiative ID (e.g., WU-123, INIT-040).  | Yes      |
| `-f, --force`           | Skip validation and promote anyway                    | No       |
| `--file &lt;path&gt;`   | Plan filename or path (resolves relative to plansDir) | No       |
...
Generate handoff prompt for sub-agent WU execution. **Mandatory after `wu:claim`** — `wu:done` rejects the auto-recorded `claim-auto` evidence and requires an explicit `wu:brief` run that generates the full prompt, checks lane occupation, and validates sizing.
