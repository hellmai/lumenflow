...
The **Evidence Store** records every tool call that passes through the [kernel](/concepts/kernel). It produces immutable, append-only records that prove what was requested, what was allowed, and what happened — regardless of whether the tool succeeded, failed, or was denied.
...
- [Kernel Runtime](/concepts/kernel) — How evidence recording fits into the tool execution pipeline
- [Policy Engine](/concepts/policy-engine) — How policy decisions are recorded in evidence
- [Scope Intersection](/concepts/scope-intersection) — How scope enforcement results are captured
