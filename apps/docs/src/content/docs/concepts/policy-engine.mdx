...
The **Policy Engine** evaluates rules at every decision point in the [kernel](/concepts/kernel) — tool calls, task claims, and task completions. Its core invariant: **a deny decision at any layer is final and cannot be reversed by any other layer.**
...
All of this is recorded in the [evidence store](/concepts/evidence) as part of the tool trace, providing a complete audit trail of _why_ an action was allowed or denied.

## Next Steps

- [Kernel Runtime](/concepts/kernel) — Where policy evaluation fits in the tool execution pipeline
- [Scope Intersection](/concepts/scope-intersection) — The permission model that runs alongside policies
- [Evidence Store](/concepts/evidence) — How policy decisions are recorded
- [Gates](/concepts/gates) — How the Software Delivery Pack uses policies for quality checks
