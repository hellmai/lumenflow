...
1. **Resolves the pack root** — from `packs/` or `packages/@lumenflow/packs/` (local), a git repository (git), or a registry (registry).
...
LumenFlow is designed for multiple packs to coexist. Each pack governs a different domain:

| Pack                | Domain                                              | Status               |
| ------------------- | --------------------------------------------------- | -------------------- |
| `software-delivery` | Software development workflows                      | Production (pack #1) |
| `infra-ops`         | Infrastructure provisioning (Terraform, Kubernetes) | Scaffold             |
| `compliance-audit`  | Compliance and audit workflows                      | Scaffold             |
| `data-pipeline`     | Data pipeline orchestration                         | Scaffold             |
| `security-ops`      | Security operations                                 | Scaffold             |
| `legal-ops`         | Legal review workflows                              | Scaffold             |

The scaffold packs declare tool names and policies but use placeholder handlers. They define the domain model — the real implementations will follow the same pattern established by the Software Delivery Pack.
