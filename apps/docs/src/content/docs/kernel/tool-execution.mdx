...
<Aside type="note">
  On Linux, the default deny overlays also include `.env` files (bind-mounted to `/dev/null`). The
  macOS backend focuses on home-directory credential directories. Both platforms protect against
  credential exfiltration by sandboxed tools.
</Aside>
