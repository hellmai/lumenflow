...
| `agent-runtime`     | Governed `agent-session` turns | In development       |
...
The `agent-runtime` pack currently exists as a scaffold in the monorepo. It establishes the
configuration namespace, the `agent:execute-turn` tool contract, static provider network scopes,
and the pack loading surface that later work units expand into full provider-backed execution.
