...
## Shipped Pack: Software Delivery
...
| `agent-runtime`     | Governed `agent-session` turns | Implemented          |
...
The `agent-runtime` pack is the second implemented pack. It provides the governed
`agent:execute-turn` contract, config-driven capability augmentation, intent-aware policy rules,
provider adapter normalization, and pack-owned `agent-session` orchestration. See the
[Agent Runtime Pack](/packs/agent-runtime) docs for the public surface.
