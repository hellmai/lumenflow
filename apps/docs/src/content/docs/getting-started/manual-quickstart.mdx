...
```bash
pnpm lumenflow --force
```

<Aside type="tip" title="Scripts are auto-managed">
  You do not need to manually add scripts. LumenFlow injects them during `pnpm lumenflow`. Re-run
  with `--force` if scripts are missing.
</Aside>
...
<Aside type="danger" title="DO NOT">
  ...
  `pnpm wu:edit` commands. Default path is `docs/tasks/wu/` (or `docs/operations/tasks/wu/` for
  arc42 projects).
</Aside>
...
| `pnpm wu:block --id WU-123 --reason "Blocked"` | Manually editing WU YAML status field  |
