...
6.  **Initialize LumenFlow**
    ...
    Treat this as a shape reference for the generated config. Manage it through the CLI and init
    flow; do not hand-edit `workspace.yaml` during setup.
...
3.  **Initialize with merge mode**
    ...
    after config changes: `pnpm lumenflow:integrate --client claude-code`
