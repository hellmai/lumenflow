...
8. SKIP-GATES AUTONOMY (WU-1142)
   - If gates fail, rerun `pnpm wu:prep --id {WU_ID}` from the worktree; it checks main safely and prints skip-gates guidance when appropriate
   - If `wu:prep` reports the failure is pre-existing on main, use: `pnpm wu:done --id {WU_ID} --skip-gates --reason "pre-existing on main" --fix-wu WU-XXXX`
