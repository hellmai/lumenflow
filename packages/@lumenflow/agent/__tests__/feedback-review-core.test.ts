...
describe('feedback-review-core', () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(join(testDir, '._legacy', 'incidents'), { recursive: true });
    await mkdir(join(testDir, '._legacy', 'memory'), { recursive: true });
  });
  ...
  describe('reviewFeedback', () => {
    beforeEach(async () => {
      ...
      await writeFile(
        join(testDir, '._legacy', 'incidents', 'test.ndjson'),
        incidents
      ...
      await writeFile(
        join(testDir, '._legacy', 'incidents', 'docs.ndjson'),
        incidents
      ...
      await writeFile(
        join(testDir, '._legacy', 'memory', 'memory.jsonl'),
    ...
    it('should handle malformed data gracefully', async () => {
      ...
      await writeFile(
        join(testDir, '._legacy', 'incidents', 'malformed.ndjson'),
