...
describe('verifyWUComplete', () => {
  ...
  describe('WU ID validation', () => {
    it('should accept valid WU ID format', () => {
      ...
      expect(result.failures[0]).toContain('Missing stamp ._legacy/stamps/WU-1234.done');
  ...
  describe('Stamp check', () => {
    it('should pass when stamp exists', () => {
      const mockRun = vi.fn(() => '');
      const mockExists = vi.fn((path) => path.includes('._legacy/stamps/WU-1234.done'));
    ...
    it('should fail when stamp is missing', () => {
      ...
      expect(result.failures[1]).toContain('Missing stamp ._legacy/stamps/WU-1234.done');
    });
  });
  ...
  describe('Complete verification', () => {
    it('should return complete when all checks pass', () => {
      ...
      const mockExists = vi.fn((path) => {
        ...
        return path.includes('._legacy/stamps/WU-1234.done');
      });
