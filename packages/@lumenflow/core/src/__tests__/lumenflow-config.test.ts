...
describe('LumenFlow Config Schema', () => {
  ...
  describe('LegacyPathsSchema', () => {
    it('should provide ._legacy defaults', () => {
      ...
      assert.strictEqual(result.base, '._legacy');
      assert.strictEqual(result.stampsDir, '._legacy/stamps');
      assert.strictEqual(result.stateDir, '._legacy/state');
    });
  });
