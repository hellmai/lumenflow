/**
 * @file legacy-migration.test.ts
 * @description Tests for ._legacy → .lumenflow migration (WU-1075)
 */

import * as fs from 'fs';
import { vi, describe, it, expect, beforeEach, afterEach } from 'vitest';
import { migrateLegacyToLumenflow, needsMigration } from '../legacy-migration.js';
...
describe('legacy-migration', () => {
  ...
  describe('migrateLegacyToLumenflow', () => {
    it('migrates ._legacy to .lumenflow when only ._legacy exists', () => {
      mockFs.existsSync.mockImplementation((p: fs.PathLike) => {
        const pathStr = String(p);
        return pathStr.endsWith('._legacy');
      });
      mockFs.renameSync.mockImplementation(() => {});

      const result = migrateLegacyToLumenflow('/project');

      expect(result.migrated).toBe(true);
      expect(result.reason).toBe('migrated');
      expect(result.fromPath).toBe('/project/._legacy');
      expect(result.toPath).toBe('/project/.lumenflow');
      expect(mockFs.renameSync).toHaveBeenCalledWith('/project/._legacy', '/project/.lumenflow');
    });
    ...
    it('uses current working directory by default', () => {
      ...
      migrateLegacyToLumenflow();

      expect(mockFs.existsSync).toHaveBeenCalledWith(`${originalCwd}/._legacy`);
    });
  });

  describe('needsMigration', () => {
    it('returns true when ._legacy exists and .lumenflow does not', () => {
      mockFs.existsSync.mockImplementation((p: fs.PathLike) => {
        const pathStr = String(p);
        return pathStr.endsWith('._legacy');
      });
