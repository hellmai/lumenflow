...
describe('force-bypass-audit', () => {
  ...
  describe('getAuditLogPath', () => {
    it('returns path relative to project root', () => {
      ...
      expect(logPath).toBe('/project/root/._legacy/force-bypasses.log');
    });
  });
  ...
  describe('logForceBypass', () => {
    ...
    it('creates ._legacy directory if it does not exist', () => {
      process.env.LUMENFLOW_FORCE = '1';
      ...
      expect(fs.mkdirSync).toHaveBeenCalledWith('/project/root/._legacy', { recursive: true });
    });
