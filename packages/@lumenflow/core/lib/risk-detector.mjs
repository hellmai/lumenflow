 ...
 *
 * Safety-critical tests (red-flag, PII, escalation) always run regardless of tier.
 *
 * Note: This is project-specific risk detection logic for 's
 * regulated domain (PHI, RLS, auth). No standard library exists for this
...
export const RISK_TIERS = Object.freeze({
  ...
  /** High-risk changes (auth, PII, RLS, migrations) - run integration tests */
  HIGH_RISK: 'high-risk',
});

/**
 * Test patterns that should ALWAYS run regardless of which files changed.
 * These are safety-critical tests that verify red-flag detection, PIIprotection,
