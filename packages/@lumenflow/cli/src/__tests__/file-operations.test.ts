 ...
 * - PII scanning for sensitive data detection
...
describe('file-write CLI', () => {
  ...
  describe('writeFileWithAudit', () => {
    ...
    it('should handle PII scanning option gracefully', async () => {
      const testFile = join(tempDir, 'phi.txt');
      // Use a valid national ID number for testing (this is a test number)
      ...
      // Should succeed regardless of PII scanning availability
      expect(result.success).toBe(true);
      // PII scanning may be disabled or unavailable, so we just check result structure
      // If PII scanning was enabled and found PII, warnings would be present
      // If PII scanning is disabled or core is not available, warnings would be undefined
      expect(result.auditLog).toBeDefined();
    });
  });
});
...
describe('file operations edge cases', () => {
  ...
  describe('parseArgs edge cases', () => {
    ...
    it('parseFileWriteArgs should parse --scan-pii flag', () => {
      const args = parseFileWriteArgs([
        'node',
        'file-write',
        '--path',
